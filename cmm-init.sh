...
mkdir -p /run/cmm
rm -f /run/cmm/src_to_f.sock /run/cmm/f_to_dst.sock /run/cmm/dst_to_src.sock
...
if [ "$APP_LANG" = cpp ]; then
  /opt/lab/apps_cpp src tun0 /run/cmm/src_to_f.sock /run/cmm/dst_to_src.sock &
else
  python3 /opt/lab/app_src.py --tun tun0 --ipc /run/cmm/src_to_f.sock --in-ipc /run/cmm/dst_to_src.sock &
fi
SRC_PID=$!
...
if [ "$APP_LANG" = cpp ]; then
  /opt/lab/apps_cpp dst /run/cmm/f_to_dst.sock /run/cmm/dst_to_src.sock &
else
  python3 /opt/lab/app_dst.py --ipc /run/cmm/f_to_dst.sock --out-ipc /run/cmm/dst_to_src.sock &
fi
DST_PID=$!

# APP_F bridges local Unix IPC to UDP backbone, and UDP back to local APP_DST IPC.
if [ "$APP_LANG" = cpp ]; then
  /opt/lab/apps_cpp f /run/cmm/src_to_f.sock /run/cmm/f_to_dst.sock "$LOCAL_BB:5000" "$REMOTE_BB:5000" &
else
  python3 /opt/lab/app_f.py \
    --src-ipc /run/cmm/src_to_f.sock \
    --dst-ipc /run/cmm/f_to_dst.sock \
